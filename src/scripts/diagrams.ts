...
const J: Color = { fill: '#fdf0ec', stroke: '#e0533d' };
const E: Color = { fill: '#eef6e8', stroke: '#5d7a4c' };
const N: Color = { fill: '#edf2f7', stroke: '#8a8a85' };
...
function node(x: number, y: number, w: number, h: number, title: string, info: string, col: Color, fs?: number): string {
  ...
  return `<g class="hs" data-info="${esc(info)}">`
    ...
    + `<text x="${x + w / 2}" y="${ty}" text-anchor="middle" font-size="${fs}" font-weight="600" fill="#1a1a1a">${tspans(lines, x + w / 2, ty, fs + 2)}</text></g>`;
}

function arrow(x1: number, y1: number, x2: number, y2: number, label?: string, col?: string): string {
  col = col || '#8a8a85';
  const m = label ? `<text x="${(x1 + x2) / 2}" y="${(y1 + y2) / 2 - 5}" text-anchor="middle" font-size="11" fill="#6a6a64" font-style="italic">${esc(label)}</text>` : '';
...
function lead(x1: number, y1: number, x2: number, y2: number): string {
  return `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="#8a8a85" stroke-width="1" stroke-dasharray="3 3"/>`;
}

function lbl(x: number, y: number, t: string, anchor?: string, col?: string, it?: boolean): string {
  return `<text x="${x}" y="${y}" text-anchor="${anchor || 'start'}" font-size="12" fill="${col || '#1a1a1a'}"${it ? ' font-style="italic"' : ''}>${esc(t)}</text>`;
}

function svg(w: number, h: number, body: string): string {
  return `<svg viewBox="0 0 ${w} ${h}" xmlns="http://www.w3.org/2000/svg">`
    + `<defs><marker id="arr" markerWidth="9" markerHeight="9" refX="7" refY="3" orient="auto"><path d="M0,0 L8,3 L0,6 Z" fill="#8a8a85"/></marker></defs>` + body + '</svg>';
}
...
D['struktur-jamur'] = () => {
  ...
  b += `<g class="hs" data-info="Tangkai (stipe) — silinder penopang tudung."><rect x="278" y="182" width="44" height="170" rx="8" fill="#fce4d8" stroke="${J.stroke}" stroke-width="2"/></g>`;
  b += `<g class="hs" data-info="Cincin (annulus) — sisa selubung parsial yang melingkari tangkai; tidak semua jamur punya."><ellipse cx="300" cy="240" rx="40" ry="9" fill="#f5c6b0" stroke="${J.stroke}" stroke-width="1.6"/></g>`;
  b += '<g class="hs" data-info="Hifa & Miselium — benang halus penyerap makanan; kumpulannya = miselium.">';
  for (let k = 0; k < 7; k++) { const hx = 300 + (k - 3) * 26; b += `<path d="M300 352 Q${hx} 380 ${hx + (k - 3) * 8} 410" fill="none" stroke="#e0533d" stroke-width="1.6"/>`; }
  b += '<rect x="200" y="352" width="200" height="60" fill="transparent"/></g>';
  b += lead(440, 120, 520, 110) + lbl(524, 113, 'Tudung', 'start', '#c2402c');
  b += lead(415, 176, 520, 176) + lbl(524, 180, 'Lamela (spora)', 'start', '#c2402c');
  b += lead(322, 250, 430, 255) + lbl(434, 258, 'Cincin', 'start', '#c2402c');
  b += lead(322, 300, 430, 305) + lbl(434, 308, 'Tangkai', 'start', '#c2402c');
  b += lead(360, 395, 470, 395) + lbl(474, 398, 'Hifa/Miselium', 'start', '#c2402c');
  return svg(600, 430, b);
};

D['hifa-septa'] = () => {
  let b = '';
  b += lbl(20, 30, '1. Hifa BERSEKAT (septat) — Ascomycota, Basidiomycota, Deuteromycota', 'start', '#c2402c');
  ...
  for (let cx = 55; cx < 520; cx += 70) b += `<circle cx="${cx}" cy="67" r="6" fill="#e0533d"/>`;
  b += '</g>' + lbl(530, 70, 'sekat', 'start', '#8a8a85', true);
  b += lbl(20, 135, '2. Hifa TIDAK BERSEKAT (asepta / senositik) — Zygomycota', 'start', '#c2402c');
  b += `<g class="hs" data-info="Hifa tidak bersekat (senositik) — tanpa sekat; banyak inti tersebar dalam satu sitoplasma. Contoh: Zygomycota."><rect x="20" y="150" width="500" height="46" rx="8" fill="#fdf0ec" stroke="${J.stroke}" stroke-width="1.6"/>`;
  for (let cx2 = 50; cx2 < 520; cx2 += 34) b += `<circle cx="${cx2}" cy="173" r="5" fill="#e0533d"/>`;
  b += '</g>' + lbl(530, 176, 'inti banyak', 'start', '#8a8a85', true);
...
D['divisi-jamur'] = () => {
  ...
  let b = '';
  for (let i = 0; i < 4; i++) { const x = 15 + i * 153; b += node(x, 20, 143, 120, it[i][0], it[i][1], J, 14); b += `<text x="${x + 71}" y="160" text-anchor="middle" font-size="11" fill="#c2402c" font-style="italic">${esc(sp[i])}</text>`; }
  return svg(640, 180, b);
};

D['repro-aseksual'] = () => {
  let b = lbl(300, 22, 'Reproduksi ASEKSUAL (tanpa peleburan inti · mitosis)', 'middle', '#c2402c');
...
D['repro-seksual'] = () => {
  ...
  b += lbl(300, 24, 'Tahap reproduksi SEKSUAL (ada peleburan inti)', 'middle', '#1a1a1a');
  b += lbl(300, 130, '→ hasil: Zigospora · Askospora · Basidiospora', 'middle', '#8a8a85', true);
...
D['siklus-basidio'] = () => {
  ...
  let b = `<circle cx="${cx}" cy="${cy}" r="${rad}" fill="none" stroke="rgba(0,0,0,0.08)" stroke-width="2" stroke-dasharray="5 5"/>`;
  b += lbl(cx, cy - 6, 'Siklus', 'middle', '#8a8a85') + lbl(cx, cy + 12, 'Basidiomycota', 'middle', '#8a8a85');
...
D['bioteknologi'] = () => {
  ...
  b += lbl(300, 196, '+ Penisilin (Penicillium notatum) · Asam sitrat (Aspergillus niger)', 'middle', '#8a8a85', true);
...
D['komponen-eko'] = () => {
  ...
  b += lbl(470, 178, 'cahaya, air, tanah,', 'middle', '#1a1a1a') + lbl(470, 194, 'udara, suhu, pH,', 'middle', '#1a1a1a') + lbl(470, 210, 'mineral, kelembapan', 'middle', '#1a1a1a');
...
D['peran-biotik'] = () => {
  ...
  let b = '';
  for (let i = 0; i < 4; i++) { const x = 15 + i * 150; b += node(x, 40, 135, 80, it[i][0], it[i][1], E, 14); if (i < 3) b += arrow(x + 135, 80, x + 150, 80, '', '#5d7a4c'); }
  b += lbl(300, 24, 'Aliran energi: Produsen → Konsumen → (Dekomposer)', 'middle', '#33563b');
...
D['interaksi'] = () => {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    b += `<g class="hs" data-info="${esc(it[i][2])}"><rect x="${x}" y="${y}" width="275" height="64" rx="10" fill="${it[i][3].fill}" stroke="${it[i][3].stroke}" stroke-width="1.6"/><text x="${x + 16}" y="${y + 38}" font-size="15" font-weight="700" fill="#1a1a1a">${esc(it[i][0])}</text><text x="${x + 259}" y="${y + 38}" text-anchor="end" font-size="15" font-family="monospace" font-weight="700" fill="#1a1a1a">${esc(it[i][1])}</text></g>`;
  }
...
D['rantai-trofik'] = () => {
  ...
  let b = '';
  for (let i = 0; i < 5; i++) { const x = 14 + i * 128; b += node(x, 70, 108, 58, org[i][0], org[i][2], E, 15); b += `<text x="${x + 54}" y="150" text-anchor="middle" font-size="11" font-weight="700" fill="#33563b">${esc(org[i][1])}</text>`; if (i < 4) b += arrow(x + 108, 99, x + 128, 99, '', '#5d7a4c'); }
  b += lbl(330, 32, 'Hitung dari PRODUSEN (Trofik I) ke atas', 'middle', '#33563b');
...
D['jaring-jaring'] = () => {
  ...
  let b = '';
  for (const [from, to] of edges) { const a = P[from], c = P[to]; b += arrow(a[0] + 44, a[1], c[0] - 44, c[1], '', '#5d7a4c'); }
  for (const k of Object.keys(P)) { const p = P[k]; b += node(p[0] - 44, p[1] - 20, 88, 40, k, k + ' — ' + info[k], E, 13); }
  b += lbl(310, 20, 'Jaring-jaring: banyak rantai tumpang tindih → 1 organisme bisa banyak jalur', 'middle', '#33563b');
...
D['piramida'] = () => {
  ...
  let b = '';
  const cols = ['#5d7a4c', '#4a6a3a', '#385828', '#26412e'];
  for (let i = 0; i < 4; i++) {
    const cx = lv[i][2], y = lv[i][3], half = lv[i][4] / 2;
    b += `<g class="hs" data-info="${esc(lv[i][1])}"><rect x="${cx - half}" y="${y}" width="${half * 2}" height="48" rx="4" fill="${cols[i]}" stroke="#1a1a1a" stroke-width="1" stroke-opacity="0.15"/><text x="${cx}" y="${y + 29}" text-anchor="middle" font-size="12.5" font-weight="600" fill="#fff">${esc(lv[i][0])}</text></g>`;
  }
  for (let i = 0; i < 3; i++) b += `<text x="500" y="${lv[i][3] + 58}" font-size="12" fill="#8a8a85" font-weight="700">↓ ×10%</text>`;
  b += lbl(300, 22, 'Piramida ENERGI — selalu tegak · hukum 10%', 'middle', '#33563b');
  b += lbl(560, 40, 'Jenis lain:', 'start', '#8a8a85', true);
  b += '<g class="hs" data-info="Piramida JUMLAH — hitung jumlah individu; bisa terbalik (1 pohon, banyak ulat)."><rect x="540" y="48" width="90" height="34" rx="6" fill="#eef6e8" stroke="#5d7a4c"/><text x="585" y="69" text-anchor="middle" font-size="11" fill="#26412e">Jumlah</text></g>';
  b += '<g class="hs" data-info="Piramida BIOMASSA — berat kering; darat tegak, perairan bisa terbalik (fitoplankton<zooplankton)."><rect x="540" y="90" width="90" height="34" rx="6" fill="#e0f2fe" stroke="#0ea5e9"/><text x="585" y="111" text-anchor="middle" font-size="11" fill="#1a1a1a">Biomassa</text></g>';
...
DAUR.air = () => {
  let b = lbl(300, 20, 'Siklus Air (Hidrologi)', 'middle', '#0ea5e9');
  ...
  b += lbl(300, 235, 'transpirasi = penguapan dari tumbuhan', 'middle', '#8a8a85', true);
  return svg(600, 250, b);
};

DAUR.karbon = () => {
  let b = lbl(300, 20, 'Siklus Karbon–Oksigen', 'middle', '#1a1a1a');
  ...
  b += arrow(240, 80, 120, 170, 'fotosintesis', '#5d7a4c');
  b += arrow(180, 198, 420, 198, 'dimakan', '#8a8a85');
  b += arrow(480, 170, 360, 80, 'respirasi', '#c2402c');
...
DAUR.nitrogen = () => {
  let b = lbl(300, 18, 'Siklus Nitrogen', 'middle', '#33563b');
  ...
  b += arrow(255, 82, 95, 150, 'fiksasi', '#5d7a4c');
  b += arrow(160, 176, 230, 176, 'nitrifikasi', '#5d7a4c');
  b += arrow(360, 176, 430, 176, 'asimilasi', '#8a8a85');
  b += arrow(430, 150, 330, 82, 'denitrifikasi → N₂', '#c2402c');
  return svg(600, 225, b);
};

DAUR.fosfor = () => {
  let b = lbl(300, 20, 'Siklus Fosfor (tidak lewat atmosfer)', 'middle', '#6a6a64');
  ...
  b += arrow(150, 118, 190, 118, 'pelapukan', '#6a6a64');
  b += arrow(310, 110, 350, 80, 'diserap', '#8a8a85');
  b += arrow(410, 96, 410, 150, 'dimakan', '#8a8a85');
  b += arrow(350, 185, 310, 210, 'mati', '#8a8a85');
  b += arrow(190, 220, 150, 146, 'kembali', '#5d7a4c');
...
export function renderDiagrams() {
  if (!document.getElementById('diag-style')) {
    ...
    st.textContent = '.hs.sel rect,.hs.sel path,.hs.sel ellipse,.hs.sel circle{stroke:#e0533d !important;stroke-width:3 !important;filter:drop-shadow(0 2px 4px rgba(224,83,61,.3))}.hs.sel text{fill:#c2402c !important}figure.diagram svg text{font-family:system-ui,sans-serif}';
